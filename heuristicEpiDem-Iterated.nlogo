globals [  
  infected-num          ;Number of individuals infected
  new-infected-num      ;Number of new individuals infected (per tick)
  died-num              ;Number of dead individuals
  infective-period      ;Infectiousness period [in ticks]
...
turtles-own [
  ...
  ticks-to-recovery     ;Tiks for total recovery
  isolated?             ;Is the individual isolated?
  immune?               ;Is the individual immunized?      
  partnered?            ;Does the individual have an encounter?
  partner               ;Identifier (WHO) of the interlocutor (nobody if no encounter)
]
...
to SetUp
  ...
  set died-num 0
  set infective-period 20      ;20 ticks (is equivalent to 20 days)
  set incubation-period 5      ;5 tiks (is equivalent to 5 days)
  set survive-probability 0.75 ;75% chance of surviving the crisis of infection  
...
to setup-common-variables
  ask turtles [
    ...
    set immune? false
    set isolated? false
    set ticks-to-recovery infective-period
...
to try-recovery
  ifelse ((random-float 1.0) > tick-alive-probability (infective-period - ticks-to-recovery) ) [
     set alive? false
     set infected? false
     set color brown
...
to pass-sanitary-filter
 if (detection-and-isolation)[ 
   if ((random-float 1.0) > tick-escape-probability (infective-period - ticks-to-recovery) ) [
    set isolated? true
    set color cyan
   ]  
 ]
  ...
  if (not partnered?) [                                          ;If the individual does not have an encounter
    ...
    if (partner != nobody) [              ;Si hay alguien cerca:
      ...
      if (partner-alive? and not partner-isolated?) [
        ...
        ask partner [                                            ;The interlocutor gets involved
          set partnered? true                                    ;The partner accepts the meeting
          set partner myself                                     ;The individual is also an interlocutor
          set heading 90                                         ;The partner faces us 
        ]
      ]
      if ((random-float 1.0) > disobedience-probability) [       ;Separation recommendation with a probability of disobedience
        separate
      ]
    ]
  ]
...
to infect-randomly
  ...
  if (partner-infected? and not infected? and not immune?) [ 
  ;If the interlocutor is infected and, in addition, the individual is not infected and is not immune:
    ;Posible contagio:
    if ((random-float 1.0) < infection-probability (distance partner)) [  
    set infected? true
    set color red
...
to-report tick-alive-probability [infection-ticks]
  let crisis-tick round(infective-period / 2)
...
to-report tick-escape-probability [infection-ticks]
  ...
  ;en otro caso, se escapa del aislamiento:
  [   
    report 1.0
  ]    
end
